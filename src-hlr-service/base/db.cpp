...
// $Id: db.cpp,v 1.1.2.6.2.4 2012/05/17 12:37:46 aguarise Exp $
// -------------------------------------------------------------------------
		...
		string dbNameI )
{
	...
	passwd = passwdI;//s
    dbName = dbNameI;//s
    const char * s = serverI.c_str();
    const char * u = userI.c_str();
    const char * p = passwdI.c_str();
    const char * d = dbNameI.c_str();
	...
	do {
		...
		if (!mysql_real_connect(dbhandle,s,u,p,d,0,NULL,0))
 	       {
