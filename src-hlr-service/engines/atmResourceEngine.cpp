...
// 
// $Id: atmResourceEngine.cpp,v 1.1.2.1.4.5.2.2 2012/03/06 12:55:30 aguarise Exp $
// -------------------------------------------------------------------------
...
int ATMResourceEngine( string &input, connInfo &connectionInfo, string *output )
{
	...
	else
	{
		...
		if ( connectionInfo.hostName == "")
				{
					//this is important for AMQ based records
					string logBuff = "using hostname from UR in connInfo" + usage_info.ceHostName;
					hlr_log (logBuff,&logStream,4);
					connectionInfo.contactString = usage_info.ceHostName;
				}
		if ( !lazyAccountCheck )
		{
