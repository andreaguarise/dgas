...
// 
// $Id: atmResourceEngine.cpp,v 1.1.2.1.4.7.4.1 2012/12/24 16:37:12 aguarise Exp $
// -------------------------------------------------------------------------
...
int ATMResourceEngine( string &input, connInfo &connectionInfo, string *output )
{
	...
	else
	{
		...
		if ( connectionInfo.contactString == "")
		{
			...
			logBuff = "using ceCertificateSubject from UR in connInfo" + usage_info.ceCertificateSubject;
	...
	if ( success )
	{
		...
		logBuff ="ATMEngine: gridJobId: " + usage_info.dgJobId + " submitted by " + usage_info.user_CertSubject;
		hlr_log ( logBuff, &logStream,4);
			...
			if ( hlrDb.errNo == 0 )
			{
				...
				if ( hlrDb.errNo == 0)
				{
					if ( result.numRows() != 0 )
					{
						...
						logBuff = "ATM Engine:"+ t.id + "," + usage_info.dgJobId + "/";
