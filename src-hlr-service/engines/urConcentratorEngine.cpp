	...
	if ( requestType == "insertRecords" )
	{
		...
		time_t time1 = time(NULL);
				...
				if ( deltaT > 0 )
				{
					...
					procSec = int2string(effProcessedPerSecond);
				}
				else
				{
					procSec = "wow";
				...
				logBuff += " sec: " + procSec + " rec/sec";
				hlr_log(logBuff,&logStream,5);
		return res;
	}
	if ( requestType == "resetRecords" )
	{
		logBuff = c->contactString + ":resetRequest:" + *input;
		hlr_log(logBuff,&logStream,6);
...
int urConcentrator::getIndex(urConcentratorIndex& indexEntry)
{
	...
	queryString += "urSourceServerDN='";//3.4.0.26
	queryString += c->contactString;
	queryString += "'";
...
int urConcentrator::insertRecords(vector<jobTransSummary>& r)
{
	...
	while ( it != r.end() )
	{
		...
		else
		{
			...
			lastInsertedRecordDate = (*it).endDate;//3.4.0.26
		}
		it++;
	}
	return 0;
...
//	queryString += r.remoteHlr + "',";
	...
	if ( (res != 0) && ( res != 1 ) )
	{
		...
		if ( res == 1062 )//MySQL duplicate
		{
			logBuff = c->hostName + ":Duplicate record with dgJobId:" + r.dgJobId + ",MysqlError:" + int2string(res);
			hlr_log(logBuff,&logStream,7);
