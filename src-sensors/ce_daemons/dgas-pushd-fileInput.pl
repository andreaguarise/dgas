...
my %configValues   = (
	lockFileName => "/var/lock/dgas/dgas-pushd-fileInput.lock",
	logFileName  => "/var/log/dgas/dgas_pushd_fileInput.log",
	mainPollInterval  => "5",
	systemLogLevel    => 7,
	maxThreadNumber   => "5",
	printAsciiLog     => "no",
	...
	successTransport1 => "0",
	successTransport2 => "0",
	recordsDir        => "/var/spool/dgas/records/",
	transportLayers       => "Transport1",
);
...
my $lastLog    = "";
my $logCounter = 0;
my @availableTransports = split( /;/, $configValues{transportLayers} );
...
while ($keepGoing) {
	...
	opendir(DIR, $configValues{recordsDir}) && closedir DIR && last;
...
while ($keepGoing) {
	...
	my $i = 0;
        while( defined(my $file = readdir(DIR)) && ( $i < $limit ) ) {
	...
	while ($keepGoing && @fileList )
	{
		...
		foreach my $transportLayer ( @availableTransports )
		{
			my $transportBuff = $configValues{"recordProducer$transportLayer"};
			my $commandBuff = "cat $configValues{recordsDir}/$fileBuff | " . $transportBuff;
			print "command: $commandBuff, protocol: $transportLayer";
			my $buff = "file:" . $fileBuff . "command:" . $commandBuff . "transport:" . $transportLayer;
            push @commandList, $buff;
		}
	}
		...
		&& ( $numOfRecords >= $maxThreadNumber ) )
	{
		...
		for ( my $i = 0 ; $i < $maxThreadNumber ; $i++ ) {
			my $commandLineBuff = pop (@commandList);
            print "$commandLineBuff\n";
            ($file[$i],$executable[$i],$transport[$i]) = ( $commandLineBuff =~ /^file:(.*)command:(.*)transport:(.*)$/);
			$status[$i]        = 0;
		}
		  ...
		  )
		{
			...
			if ($pid) {
				...
				if ( $file[$threadNumber] ) {
					&printLog( 8, "Spawning new process:$threadNumber on record: $file[$threadNumber]"
					);
...
sub parseConf {
	...
	while (<FILE>) {
		...
		if (/^pushdFileInputTransportLayer\s*=\s*\"(.*)\"$/) {
			$configValues{transportLayers} = $1;
		}
		...
		if (/^pushdFileInputRecordsDir\s*=\s*\"(.*)\"$/) {
