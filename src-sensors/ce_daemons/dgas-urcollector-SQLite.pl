...
sub putBuffer
{
	...
	if ( $_[2] < $bufferTimestampGlobal-600 )
	{
		&printLog( 1, "ASSERT Write in Buffer $_[0]; minor timestamp. Not Updating Buffer.", 1 );
		goto START;
	}
	if ( $keepGoing == 1 )
	{
...
sub processLrmsLogs
{
	...
	while ( $keepGoing )
	{
		...
		while ( $keepGoing && @sortedLrmsLogFiles )
		{
			...
			{
				&printLog( 8, "File is empty, skipping!" );
				next;
			}
			...
			if ( $logFMod{$thisLogFile} < $ignoreJobsLoggedBefore )
			{
				...
				# hence we stop here!
				&printLog( 8, "skipping $thisLogFile because too early", 1 );
				next;
			}
			elsif ( $logFMod{$thisLogFile} < $startTimestamp )
			{
				&printLog( 8, "skipping $thisLogFile because already processed",
					1 );
				next;
			}
			else
...
sub processLrmsLogFile
{
	...
	while ( $line = <LRMSLOGFILE> )
	{
		if ( !$keepGoing )
		{
			&printLog( 7, "Stop processing $filename ..." );
			close LRMSLOGFILE;
			return 1;
		}
...
/\s*\"timestamp=(\d{4})[-\/](\d{2})[-\/](\d{2})\s(\d{2}):(\d{2}):(\d{2})\"\s*/
				  )
				{
					...
					&printLog( 6,
"Blah log: lrmsID=$lrmsJobID with timestamp(UTC)=$1-$2-$3 $4:$5:$6 ($ceEntryTimestamp)"
					);
