...
sub parseUR_lsf {
    ...
    if ( scalar(@new) == 0 )
    {
    			&printLog(6, "Found problematic LSF UR line, using alternate parser.");
                $URString =~ s/"\s"/DSEPARATOR/g;
                $URString =~ s/(\d)\s(\d)/$1DSEPARATOR$2/g;
                $URString =~ s/(\d)\s(\d)/$1DSEPARATOR$2/g;
                $URString =~ s/(-\d)\s(\d)/$1DSEPARATOR$2/g;
                $URString =~ s/(\d)\s(-\d)/$1DSEPARATOR$2/g;
                $URString =~ s/(\d)\s"/$1DSEPARATOR/g;
                $URString =~ s/"\s(\d)/DSEPARATOR$1/g;
                $URString =~ s/"\s(-\d)/DSEPARATOR$1/g;
                $URString =~ s/"(-\d)\s/$1DSEPARATOR/g;
                @new = split /DSEPARATOR/,$URString ;
    }
    ...
    $urGridInfo{lsf_sla}    = $new[ 65 +$shift2];
	...
	sub composeATMtags {
		...
		foreach my $def (@$ATMdefs) {
			...
			if ( exists $hashBuff{$1} ) {
				$def =~ s/\$(\w+)/$hashBuff{$1}/g;
				push( @$atmTags, $def );
				&printLog( 7, "added in record:$def" );
			}
			else
			{
				push( @$atmTags, $def );
...
"Removing temporary buffer $tmpBuffer ... not required anymore!";
	...
	sub putTmpBuffer {
		...
		close(OUT);
		&printLog( 6, "TMP BUFFER Writing tmp buffer $buffName with '$_[1]:$_[2]:$_[3]'" );
		return 0;
	}
